...
#define ESP_WIFI_PASS "12345678" // 必须大于等于8位以满足WPA2要求
...
void wifi_init_softap(void) {
    ...
    wifi_config_t wifi_config = {
        .ap = {
            ...
            .max_connection = 4,
            .authmode = WIFI_AUTH_WPA2_PSK // macOS 可识别的安全设置
        },
    };

    // 核心修改：设置模式为 AP，且配置接口必须为 WIFI_IF_AP
    ESP_ERROR_CHECK(esp_wifi_set_mode(WIFI_MODE_AP));
    ESP_ERROR_CHECK(esp_wifi_set_config(WIFI_IF_AP, &wifi_config));
    ESP_ERROR_CHECK(esp_wifi_start());

    ESP_LOGI(TAG, "WiFi AP 模式启动完成。SSID: %s", ESP_WIFI_SSID);

    // AP 启动后直接启动 Web Server
    start_webserver();
}
